...
def run_chatbot():
    ...
    Loads/builds the FAISS index and then enters a loop for user queries.
    """
    # Ensure brand documents directory exists for the simulated GraphRAG agent
    ...
    # Load or build FAISS index and retrieve all indexed conversations
    ...
    if faiss_index is None:
        ...
        # We can still run the brand knowledge agent, but customer conversations will fail.
        # For simplicity, we'll allow it to continue, but handle the no-index case in the routing.
